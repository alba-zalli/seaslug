extends Node

@export var world_dialogue: DialogueResource = preload(
	"res://dialogue/worldmode.dialogue"
)
@export var visit_button: Button

var balloon_open: bool = false

# Remembers whichever region was hovered most recently.
var last_hovered_region: String = ""

const REGION_SCENES: Dictionary = {
	"indian_ocean": "res://bgscenes/IndianOceanScene.tscn",
	"east_pacific": "res://bgscenes/EastPacificScene.tscn",
	"west_pacific": "res://bgscenes/WestPacificScene.tscn",
}


func _ready() -> void:
	print("CONTROLLER SCRIPT IS RUNNING")

	DialogueManager.dialogue_ended.connect(
		_on_dialogue_ended
	)

	var regions: Array[Node] = (
		get_tree().get_nodes_in_group("map_regions")
	)

	print("Found regions: ", regions.size())

	for region: Node in regions:
		if region.has_signal("region_hovered"):
			region.region_hovered.connect(
				_on_region_hovered
			)

	if visit_button != null:
		visit_button.disabled = true

	balloon_open = true

	DialogueManager.show_dialogue_balloon(
		world_dialogue,
		"intro",
		[self]
	)


func _on_region_hovered(dialogue_title: String) -> void:
	last_hovered_region = dialogue_title

	print("Hovered region: ", dialogue_title)

	if visit_button != null:
		visit_button.disabled = false

	if balloon_open:
		return

	balloon_open = true

	DialogueManager.show_dialogue_balloon(
		world_dialogue,
		dialogue_title,
		[self]
	)


func _on_dialogue_ended(_resource: DialogueResource) -> void:
	balloon_open = false


func _on_button_pressed() -> void:
	print("========== BUTTON CLICKED ==========")
	print("last_hovered_region = ", last_hovered_region)

	if last_hovered_region.is_empty():
		print("NO REGION SELECTED")
		return

	if not REGION_SCENES.has(last_hovered_region):
		push_error(
			"REGION NOT FOUND: "
			+ last_hovered_region
		)
		return

	var destination_path: String = str(
		REGION_SCENES[last_hovered_region]
	)

	print("SENDING TO LOADER: ", destination_path)

	SceneLoader.load_scene(destination_path)
